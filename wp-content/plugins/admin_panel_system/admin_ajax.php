...
if ( isset($_REQUEST) ){
	...
	if(isset($_REQUEST['send_message_unlogged'])){
		global $wpdb, $info;
		$user_to_ask = sanitize_text_field($_POST['fields']['mail_unlogged_user']['value']);
		$message_to_ask = sanitize_text_field($_POST['fields']['message']['value']);
		$camp_unlogg_id = sanitize_text_field($_POST['fields']['campamento']['value']);
		$camp_title = get_the_title(sanitize_text_field($_POST['fields']['campamento']['value']));
		$author_id = get_post_field('post_author', $camp_unlogg_id);
		$mail_unlogged = $author_id ? get_the_author_meta( 'user_email', $author_id ) : get_option("admin_email");
		$nombre = $author_id ? get_the_author_meta( 'display_name', $author_id ) : 'Administrador';
		$_data = [
			"nombre" => $nombre,
			"mensaje" => "Hola $nombre,<br><br>Tienes un nuevo mensaje para leer y responder (el mensaje fue hecho por un usuario sin cuenta, por lo que la gestión será por correo electrónico)<br><b>Correo</b>: $user_to_ask.<br><b>Mensaje</b>: $message_to_ask.<br><b>Campamento</b>: $camp_title",
		];
		$this->mail($mail_unlogged, __("Nueva solicitud de información en TOP Campamentos", 'adpnsy'), "notificacion", $_data, $info);	
	}
	...
	if(isset($_REQUEST['GET_DATA_']) && $_REQUEST['GET_DATA_'] == 5){
		...
		if($query_res_1->have_posts() && $query_res_1->post->ID !== $id_camp_current_ce){
			...
			if(!$id_camp_current_ce){
				...
				$update_status_camp = array(
					'ID' => $camp_id,
					'post_status' 	=> $p_or_d,
				);
				$res_updt_status = wp_update_post($update_status_camp);
			} 
			if($camp_id){
				foreach($form_data as $k => $v){
					if($k == 'camp_title' || $k == 'id_camp_current'){
						...
						if(count(array_diff_assoc($plazas_sale, $plazas_sale_old))){
						...
						$cluecamp_new = stripslashes($cluecamp_new);
						...
						if($v){
							...
							if($meta_old_if_n_dir != stripslashes($v)){
								if(isset($form_data['valueApiPlacesSave']) && !empty($form_data['valueApiPlacesSave'])){
									$apiPlacesValue = json_decode(stripslashes($form_data['valueApiPlacesSave']));
									...
									if($meta_old_if_lt_dir != $apiPlacesValue->lat){
										$meta_if = update_post_meta($camp_id, 'direction_lat_camp', $apiPlacesValue->lat);
										if(!$meta_if) {
											$mensajes_ajax[] = "Ha ocurrido un error al intentar almacenar la información en el metacampo ".$k;
										}
									}
									if($meta_old_if_lg_dir != $apiPlacesValue->lng){
										$meta_if = update_post_meta($camp_id, 'direction_log_camp', $apiPlacesValue->lng);
										if(!$meta_if) {
											$mensajes_ajax[] = "Ha ocurrido un error al intentar almacenar la información en el metacampo ".$k;
										}
									}
							...
							update_post_meta($camp_id, $k, '');
							update_post_meta($camp_id, 'direction_lat_camp', '');
							update_post_meta($camp_id, 'direction_log_camp', '');
						}
					}else{
						if(is_array($v)){
							$meta_data_trat_ = array();
	...
	if(isset($_REQUEST['csv_reservas']) && wp_verify_nonce($_REQUEST['csv_reservas'], get_current_user_id())){
		...
		if($result){
			foreach($result as $k => $dt){
				$fechas = explode(' | ', $dt->fechas_reserva);
				$f1 = explode('/', $fechas[0]);
				$f1 = $f1[1].'-'.$f1[0].'-'.$f1[2];
				$f2 = explode('/', $fechas[1]);
				$f2 = $f2[1].'-'.$f2[0].'-'.$f2[2];
				...
				$result_print[$k][] = $fechas;
				$result_print[$k][] = $dt->plazas;
				$result_print[$k][] = (($dt->precio/15)*85);
				$meta_data_ord = get_post_meta($dt->id_orden);
				$data_kids = [];
				$result_print[$k][] = (isset($meta_data_ord['billing_document_responsible']) && $meta_data_ord['billing_document_responsible'][0]) ? $meta_data_ord['billing_document_responsible'][0] : '';
				if(isset($meta_data_ord['num_campistas_form'])){
					$num_kids = ($meta_data_ord['num_campistas_form'][0]) ? (int)$meta_data_ord['num_campistas_form'][0] : '';
					$itx = 1;
					for($i=0;$i<$num_kids;$i++){
						$comments = ($meta_data_ord['comentarios_campista'.$itx][0]) ? $meta_data_ord['comentarios_campista'.$itx][0] : 'Sin comentarios';
						$dates = ($meta_data_ord['fecha_nacimiento_campista'.$itx][0]) ? $meta_data_ord['fecha_nacimiento_campista'.$itx][0] : '';
						$name_c = ($meta_data_ord['nombre_campista'.$itx][0]) ? $meta_data_ord['nombre_campista'.$itx][0] : '';
						$genero_c = ($meta_data_ord['genero_campista'.$itx][0]) ? $meta_data_ord['genero_campista'.$itx][0] : '';
						$age_kids = '';
						if($dates){
							$dk1 = new DateTime();
							$dk2 = new DateTime($dates);
							$diff = $dk1->diff($dk2);
							if($diff->days < 365){
								$age_kids = 'Menos de un año';
							}else{
								$age_kids = (date('Y') - date('Y',strtotime($dates))) . ' años';
							}
							$dates = explode('-', $dates);
							$dates = $dates[2].'-'.$dates[1].'-'.$dates[0];
						}
						if(!in_array(('Nombre campista N° '.$itx), $tabs_data_kids)) $tabs_data_kids[] = 'Nombre campista N° '.$itx;
						if(!in_array(('Fecha nacimiento campista N°  '.$itx), $tabs_data_kids)) $tabs_data_kids[] = 'Fecha nacimiento campista N°  '.$itx;
						if(!in_array(('Edad campista N° '.$itx), $tabs_data_kids)) $tabs_data_kids[] = 'Edad campista N° '.$itx;
						if(!in_array(('Genero campista N° '.$itx), $tabs_data_kids)) $tabs_data_kids[] = 'Genero campista N° '.$itx;
						if(!in_array(('Comentarios campista N° '.$itx), $tabs_data_kids)) $tabs_data_kids[] = 'Comentarios campista N° '.$itx;
						$result_print[$k][] = $name_c;
						$result_print[$k][] = $dates;
						$result_print[$k][] = $age_kids;
						$result_print[$k][] = ucfirst($genero_c);
						$result_print[$k][] = $comments;
						$itx++;
					}
				}
			}
		...
		echo "\xEF\xBB\xBF";
		$tab1 = array("Orden", "Nombre", "Correo", "Telefono", "Campamento", "Fechas", "Plasas solicitadas", "Pago pendiente", "Documento Responsable");
		$tabs_csv = array_merge($tab1, $tabs_data_kids);
		fputcsv($outputBuffer, $tabs_csv, ";");
	...
	if(isset($_REQUEST['visBook']) && $_REQUEST['visBook'] == 4){
		...
		if($sql){
			...
			$meta_data_ord = get_post_meta($book_id);
			$data_kids = [];
			$doc_res = (isset($meta_data_ord['billing_document_responsible']) && $meta_data_ord['billing_document_responsible'][0]) ? $meta_data_ord['billing_document_responsible'][0] : '';
			$data_kids['Documento Responsable'] = $doc_res;
			if(isset($meta_data_ord['num_campistas_form'])){
				$num_kids = ($meta_data_ord['num_campistas_form'][0]) ? (int)$meta_data_ord['num_campistas_form'][0] : '';
				$itx = 1;
				for($i=0;$i<$num_kids;$i++){
					...
					$data_kids['Nombre campista N° '.$itx] = $name_c;
					$data_kids['Fecha nacimiento campista N° '.$itx] = $dates;
					$data_kids['Edad campista N° '.$itx] = $age_kids;
					$data_kids['Genero campista N° '.$itx] = ucfirst($genero_c);
					$data_kids['Comentarios campista N° '.$itx] = $comments;
					$itx++;
				}
			}
			$sql->info_campists = $data_kids;
			$r['r'] = true;
	...
	if(isset($_REQUEST['mailBook']) && $_REQUEST['mailBook'] == 1){
		...
		$r['r'] = true;
		$r['m'] = 'Los datos se guardaron de manera correcta';
	}
	...
	if(isset($_REQUEST['dashData']) && $_REQUEST['dashData'] == 5){
		...
		if($comision === false || $comision === '' || $comision === null) $comision = 15;
		$r['r'] = true;
	...
	if(isset($_REQUEST['dashData']) && $_REQUEST['dashData'] == 6){
		...
		if(update_option('comi_booking', $comision)){
			$r['r'] = true;
			$r['m'] = 'Comisión configurada de manera correcta'; 
		}else{
	...
	if(isset($_REQUEST['reserva']) && $_REQUEST['reserva'] == '_look_'){
    	...
    	$_q = "SELECT
        	...
        	$extra
        ";
		$rtrn_booking_data = $wpdb->get_results($_q);
		foreach($rtrn_booking_data as $rkd => $rvd){
			$id_order_woo_d = $rvd->id;
			$order_woo_d_meta = get_post_meta($id_order_woo_d);
			$rvd->correo = $order_woo_d_meta['_billing_email'][0];
			$rvd->telefono = $order_woo_d_meta['_billing_phone'][0];
			$rvd->dni = $order_woo_d_meta['_billing_document_responsible'][0];
		}
