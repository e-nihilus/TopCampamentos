...
class Data {
	...
	public function sync_rating_source_meta_by_id( $source = false, $source_type = false, $source_id = false ) {
		...
		$source_type_data = jet_reviews()->settings->get_post_type_data( $source_type );
		$table_name = jet_reviews()->db->tables( 'reviews', 'name' );
		 ...
		 * Maybe update average rating post meta field
		 */
		if ( filter_var( $source_type_data['metadata'], FILTER_VALIDATE_BOOLEAN ) && ! empty( $average_rating_percent ) ) {
			update_post_meta( $source_id, $source_type_data['metadata_rating_key'], ( $average_rating_percent / 100 ) * $source_type_data['metadata_ratio_bound'] );
