(function ($) {
	...
	var MapHandler = function ($scope, $) {
		...
		(function initMap() {
			var latlng = new google.maps.LatLng(locations[0]['lat'], locations[0]['lang']);
				mapOptions = {
					zoom:               zoom,
					center:             latlng,
					mapTypeId:          mapType,
			...
			for (i = 0; i < locations.length; i++) {
				var icon           = '',
					lat            = locations[i]['lat'],
					lng            = locations[i]['lang'],
					info_win       = locations[i]['infowindow'],
					title          = locations[i]['title'],
					description    = locations[i]['description'],
					icon_type      = locations[i]['marker_type'],
					icon_url       = locations[i]['icon_url'],
					icon_size      = locations[i]['icon_size'],
					iw_on_load     = locations[i]['iw_open'];
