...
function pp_activate_license() {
	...
	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( 403 === $code ) {
				$message = esc_html__( 'An error occurred while activating license. The request is getting blocked by a security plugin or security settings on server.', 'powerpack' );
			} else {
				$message = sprintf( esc_html__( 'An error occurred, please try again. Status: %1$s %2$s', 'powerpack' ), $code, $response_msg );
			}
		}
	} else {
...
function pp_deactivate_license() {
	...
	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$response_msg = wp_remote_retrieve_response_message( $response );
			if ( 403 === $code ) {
				$message = esc_html__( 'An error occurred while deactivating license. The request is getting blocked by a security plugin or security settings on server.', 'powerpack' );
			} else {
				$message = sprintf( esc_html__( 'An error occurred, please try again. Status: %1$s %2$s', 'powerpack' ), $code, $response_msg );
			}
		}
...
function pp_in_plugin_update_message( $plugin_data, $response ) {
	...
	if ( 'valid' !== $data->license ) {
		...
		$main_msg = sprintf( esc_html__( 'Please activate the license to enable automatic updates for this plugin. License status: %s', 'powerpack' ), $data->license );
		...
		$message .= esc_html__( 'Buy Now', 'powerpack' );
		$message .= ' &raquo;</a>';
		$message .= '</p>';
...
function pp_get_license_error( $license_data ) {
	...
	switch ( $license_data->error ) {
		...
		case 'expired':
			$message = sprintf(
				esc_html__( 'Your license key expired on %s.', 'powerpack' ),
				date_i18n( get_site_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
			);
			break;

		case 'revoked':
			$message = esc_html__( 'Your license key has been disabled.', 'powerpack' );
			break;

		case 'missing':
			$message = esc_html__( 'Invalid license.', 'powerpack' );
			break;

		case 'invalid':
		case 'site_inactive':
			$message = esc_html__( 'Your license is not active for this URL.', 'powerpack' );
			break;

		case 'item_name_mismatch':
			$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'powerpack' ), POWERPACK_ITEM_NAME );
			break;

		case 'no_activations_left':
			$message = esc_html__( 'Your license key has reached its activation limit.', 'powerpack' );
			break;

		default:
			$message = sprintf( esc_html__( 'An error occurred, please try again. Status: %s', 'powerpack' ), $license_data->error );
			break;
	}
