...
class Module extends Module_Base {
	...
	public function process_login() {
		if (
			...
			! wp_verify_nonce( wp_unslash( $_POST['ppe-lf-login-nonce'] ), 'pp_login_nonce' ) ) {
				wp_send_json_error( esc_html__( 'Invalid data.', 'powerpack' ) );
		}
			...
			if ( ! empty( $recaptcha_secret_key ) && ! empty( $recaptcha_site_key ) ) {
				if ( version_compare( phpversion(), '5.3', '>=' ) ) {
					...
					if ( ! $validate->is_success() ) {
						wp_send_json_error( esc_html__( 'Error verifying reCAPTCHA, please try again.', 'powerpack' ) );
					}
				} else {
					wp_send_json_error( esc_html__( 'reCAPTCHA API requires PHP version 5.3 or above.', 'powerpack' ) );
				}
			} else {
				wp_send_json_error( esc_html__( 'Your reCAPTCHA Site or Secret Key is missing!', 'powerpack' ) );
			}
		}
		...
		if ( isset( $_POST['username'], $_POST['password'] ) ) {
			try {
				...
				if ( $validation_error->get_error_code() ) {
					throw new \Exception( '<strong>' . esc_html__( 'Error:', 'powerpack' ) . '</strong> ' . $validation_error->get_error_message() );
				}

				if ( empty( $creds['user_login'] ) ) {
					throw new \Exception( '<strong>' . esc_html__( 'Error:', 'powerpack' ) . '</strong> ' . esc_html__( 'Username is required.', 'powerpack' ) );
				}
			...
			wp_send_json_error( esc_html__( 'Username or password is missing.', 'powerpack' ) );
		}
	}
	...
	public function process_social_login() {
		...
		if ( ! isset( $_POST['provider'] ) ) {
			wp_send_json_error( esc_html__( 'Provider was not set.', 'powerpack' ) );
		}
	...
	private function process_facebook_login() {
		$err_unauth_access = esc_html__( 'Unauthorized access.', 'powerpack' );
		...
		if ( ! empty( $userdata ) ) {
			if ( $validate_password && isset( $data['password'] ) ) {
				if ( ! wp_check_password( $data['password'], $userdata->user_pass, $userdata->ID ) ) {
					wp_send_json_error( esc_html__( 'Password does not match.', 'powerpack' ) );
				}
			}
			...
			if ( ! apply_filters( 'pp_login_form_social_login_registration', get_option( 'users_can_register' ) ) ) {
				wp_send_json_error( esc_html__( 'New registrations are currently disabled.', 'powerpack' ) );
			}
	...
	private function retrieve_password() {
		...
		if ( empty( $login ) ) {
			...
			$this->form_error = esc_html__( 'Enter a username or email address.', 'powerpack' );
		...
		if ( ! $user_data ) {
			$this->form_error = esc_html__( 'Invalid username or email.', 'powerpack' );
		...
		if ( ! $allow ) {
			...
			$this->form_error = esc_html__( 'Password reset is not allowed for this user', 'powerpack' );
	...
	public function process_reset_password() {
		...
		if ( empty( $posted_fields['password_1'] ) ) {
			$this->form_error = esc_html__( 'Please enter your password.', 'powerpack' );
		} elseif ( $posted_fields['password_1'] !== $posted_fields['password_2'] ) {
			$this->form_error = esc_html__( 'Passwords do not match.', 'powerpack' );
		}
		...
		wp_send_json_error( esc_html__( 'Unknown error', 'powerpack' ) ); //phpcs:ignore 
	}
	...
	public function check_password_reset_key( $key, $login ) {
		...
		if ( is_wp_error( $user ) ) {
			$this->form_error = esc_html__( 'This key is invalid or has already been used. Please reset your password again if needed.', 'powerpack' );
			return false;
		}
	...
	public function register_styles() {
		wp_register_style(
			'widget-pp-login-form',
			$this->get_css_assets_url( 'widget-login-form', null, true, true ),
			[],
